...
/*        // 모든 도시의 관광지를 리스트를 합칩니다
        List<String> allAttractions = itinerary.stream()
                .flatMap(city -> city.getAttractions().stream())
                .collect(Collectors.toList());
        // 중복되는 관광지를 제거합니다.
        List<String> uniqueAttractions = allAttractions.stream()
                .distinct()
                .collect(Collectors.toList());
        // 관광지 이름을 알파벳 순으로 정렬합니다.
        List<String> sortAttractions = uniqueAttractions.stream()
                .sorted()
                .collect(Collectors.toList());
        // 최종 여행지 리스트를 출력합니다.
        System.out.println("최종 여행지 리스트: ");
        sortAttractions.forEach(System.out::println);*/
