...
public class Thread6 {
    ...
    public static void main(String[] args) {
        ...
        // 우선순위 부여하기
        // 스레드 스케즐러가 스레드들 사이에서 실행순서를 결정하는데 사용되는 힌트
        // OS(운영 체제)에 있는 스케쥴링 정책에 따라 CPU의 자원을 할당받음
        // 어느 정도 빈도의 영향을 가지게 됨
        // 숫자가 클수록 높은 우선운위를 가짐 => 보장된 것은 아니라 OS 에게 힌트만 줌 (무시당할 수 있음)
        thread0.setPriority(1);     // 최소 우선순위
        thread0.setPriority(5);     // 중간 우선순위
        thread0.setPriority(10);    // 최대 우선순위
        ...
        /*
        - 프로그램의 행동을 예측가능하게 만드는 데 사용해서는 안됨 (우선순위 무시당할 수 있음)
        - 낮은 우선 순위의 스레드가 실행기회를 박탈당할 수 있으므로, 리소스 관리 필요에따라 신중하게 사용
         */
