...
public class Object1 {
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("배열여부 : " +  aClass.isArray());
        System.out.println("인터페이스여부 : " +  aClass.isInterface());
        System.out.println("기본타입여부 : " +  aClass.isPrimitive());

        System.out.println("부모클래스 : " +  aClass.getSuperclass());

        // 타입 정보률 조회 : 객체의 정확한 타입을 확인 가능
        ...
        object.wait();          // 현재 스레드 일시중단하고 깨울때까지 기다림
        object.wait(1000);  // 스레드 중단하고 n 밀리초 기다림
        object.wait(100, 100);  // 스레드 중단하고, n밀리초 , m나노초 기다림

        // object.clone();         // 객체의 복제본을 생성하는 메서드
        // object.finalize();      // 더 이상 사용하지 않는 메서드
        // -> Garbage Collector 에 의해 객체가 삭제되기 전에 호출되는 메서드
    }
}
