...
public class Ex4 {
    public static void main(String[] args) {
        Set<String> userAFriends = new HashSet<>(List.of("B","C","D","E"));
        Set<String> userBFriends = new HashSet<>(List.of("A","C","D","F","G"));

        // B의 친구 집합에서 A의 친구 집합(교집합 부분)을 제거 (차집합)
        Set<String> recommendFriends = new HashSet<>(userBFriends);
        recommendFriends.removeAll(userAFriends);
        // 자기자신(A) 제외
        recommendFriends.remove("A");

        System.out.println("recommendFriends = " + recommendFriends);
    }
}
